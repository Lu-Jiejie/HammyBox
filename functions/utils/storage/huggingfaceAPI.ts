...
interface PreuploadResult {
  files?: Array<{ uploadMode?: string }>;
}

export class HuggingFaceAPI {
  token: string;
  repo: string; // 格式: username/repo-name
  isPrivate: boolean;
  baseURL: string;

  constructor(token: string, repo: string, isPrivate = false) {
    this.token = token;
  ...
  async repoExists(): Promise<boolean> {
    try {
      ...
      console.error('Error checking repo:', (error as Error).message);
      return false;
    }
  }
   ...
   * 创建仓库（如果不存在）
   */
  async createRepoIfNotExists(): Promise<boolean> {
    try {
      ...
      console.error('Error creating repo:', (error as Error).message);
      return false;
    }
  }
  ...
  async preupload(
    ...
    return (await response.json()) as PreuploadResult;
  }

  /**
   * 步骤2: LFS Batch - 获取上传 URL
   */
  async lfsBatch(oid: string, fileSize: number): Promise<LfsBatchResult> {
    ...
    return (await response.json()) as LfsBatchResult;
  }

  /**
   * 步骤3: 上传文件到 LFS 存储
   * @param uploadAction 上传动作信息
   * @param file 文件
   * @param oid 文件的 SHA256 哈希
   */
  async uploadToLFS(uploadAction: LfsUploadAction, file: File | Blob, oid: string): Promise<boolean> {
  ...
  async uploadMultipart(
    uploadAction: LfsUploadAction,
    file: File | Blob,
    oid: string,
  ): Promise<boolean> {
    const { href: completionUrl, header } = uploadAction;
    const chunkSize = parseInt(header?.chunk_size ?? '', 10);
    ...
    for (const part of parts) {
      ...
      completeParts.push({ partNumber: parseInt(part, 10), etag });
    }
   ...
   * @param fileSize 文件大小
   * @param filePath 存储路径
   * @param sha256 文件的 SHA256 哈希
   * @param fileSample 文件前512字节的 base64
   */
  async getLfsUploadInfo(
    fileSize: number,
    filePath: string,
    sha256: string,
    fileSample: string,
   ...
   * @param precomputedSha256 前端预计算的 SHA256（可选，传入可避免后端计算）
   */
  async uploadFile(
    file: File | Blob,
    filePath: string,
    commitMessage = 'Upload file',
    precomputedSha256: string | null = null,
  ): Promise<{
    success: boolean;
    filePath: string;
    fileUrl: string;
    ...
    try {
      ...
      console.error('HuggingFace upload error:', (error as Error).message);
      throw error;
    }
  }
   ...
   * 删除文件
   */
  async deleteFile(filePath: string, commitMessage = 'Delete file'): Promise<boolean> {
    const url = `${this.baseURL}/api/datasets/${this.repo}/commit/main`;
   ...
   * 获取文件内容（用于私有仓库代理）
   */
  async getFileContent(filePath: string): Promise<Response> {
    const fileUrl = `${this.baseURL}/datasets/${this.repo}/resolve/main/${filePath}`;
  ...
  static getMetadataFileSize(metadata: Record<string, unknown> = {}): number | null {
