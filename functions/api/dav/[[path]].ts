...
export async function onRequest(context: PagesContext): Promise<Response> {
  ...
  const authResponse = await checkAuth(request, env as Env);
  ...
  switch (modifiedRequest.method) {
    ...
    case 'PROPFIND':
      return handlePropfind(modifiedRequest, env as Env);
    case 'PUT':
      return handlePut(modifiedRequest, env as Env);
    case 'DELETE':
      return handleDelete(modifiedRequest, env as Env);
    case 'GET':
      return handleGet(modifiedRequest, env as Env);
    case 'MKCOL':
...
async function getApiHeaders(env: Env): Promise<Record<string, string>> {
...
async function handlePropfind(request: Request, env: Env): Promise<Response> {
  ...
  try {
    const dir =
      path === '/'
        ? ''
        : path.substring(1, path.endsWith('/') ? path.length - 1 : path.length);
    const contents = await fetchDirectoryContents(dir, env, request);
...
function generateDirectoryListingHtml(
  ...
  for (const file of contents.files) {
    ...
    const fileSize =
      file.metadata && file.metadata['FileSize'] ? `${file.metadata['FileSize']} MB` : 'N/A';
...
function generateWebDAVXml(basePath: string, contents: DirectoryContents): string {
  let responses = '';
...
function createCollectionXml(path: string): string {
...
function createFileXml(file: any): string {
  ...
  const timestamp =
    file.metadata && file.metadata['TimeStamp']
      ? new Date(Number(file.metadata['TimeStamp'])).toUTCString()
      : new Date().toUTCString();
  const contentType =
    file.metadata && file.metadata['FileType']
      ? file.metadata['FileType']
      : 'application/octet-stream';
  return `<D:response><D:href>${encodeURI(`/dav/${file.name}`)}</D:href><D:propstat><D:prop><D:displayname>${file.name
    .split('/')
    .pop()}</D:displayname><D:resourcetype/><D:creationdate>${timestamp}</D:creationdate><D:getlastmodified>${timestamp}</D:getlastmodified><D:getcontentlength>${fileSize}</D:getcontentlength><D:getcontenttype>${contentType}</D:getcontenttype></D:prop><D:status>HTTP/1.1 200 OK</D:status></D:propstat></D:response>`;
}