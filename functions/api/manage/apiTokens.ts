...
import type { Env, ConfigStore, ApiToken, ApiTokenPermission } from '../../types';
...
// 获取所有API Token
async function getApiTokens(db: ConfigStore): Promise<{ tokens: any[] }> {
  ...
  const tokenList = toKeep.map((t: any) => ({
    id: t.id,
    name: t.name,
    owner: t.owner,
...
// 删除API Token
export async function deleteApiToken(db: ConfigStore, tokenId: string): Promise<Record<string, any>> {
  const settingsStr = await db.get('manage@sysConfig@security');
  const settings: any = settingsStr ? JSON.parse(settingsStr) : {};
...
export async function getTokenPermissions(db: ConfigStore, token: string): Promise<ApiTokenPermission[] | null> {
  const settingsStr = await db.get('manage@sysConfig@security');
  const settings: any = settingsStr ? JSON.parse(settingsStr) : {};
  const tokens = settings.apiTokens?.tokens || {};
