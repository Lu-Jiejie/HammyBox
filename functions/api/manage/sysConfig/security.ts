...
export async function onRequest(context: RequestContext): Promise<Response | undefined> {
...
export async function getSecurityConfig(db: ConfigStore, env: Env): Promise<SecurityConfig> {
  ...
  const upload = {
    moderate: {
      ...
      moderateContentApiKey:
        kvUpload.moderate?.moderateContentApiKey || kvUpload.moderate?.apiKey || env.ModerateContentApiKey || '',
  ...
  return settings as SecurityConfig;
}