...
// 判断请求域名是否在允许的域名列表中
export function isDomainAllowed(context: FileContext): boolean {
...
export function isFromPublicBrowse(Referer: string | null, origin: string): boolean {
...
// 设置Range请求相关头部
export function setRangeHeaders(
  headers: Headers,
  rangeStart: number,
  rangeEnd: number,
  totalSize: number
...
// 处理HEAD请求的公共函数
export function handleHeadRequest(headers: Headers, etag: string | null = null): Response {
...
export function isTgChannel(imgRecord: { metadata?: FileMetadata | null }): boolean {
  return (
    imgRecord.metadata?.Channel === 'Telegram' || imgRecord.metadata?.Channel === 'TelegramNew'
  );
}

// 图片可访问性检查
export async function returnWithCheck(context: FileContext, imgRecord: FileRecord): Promise<Response> {
...
function unauthorizedPreviewResponse(): Response {
...
export async function return404(url: URL): Promise<Response> {
...
export async function returnBlockedResponse(
  url: URL,
  reason = 'blocked'
): Promise<Response> {
  // 尝试获取静态拦截图片
...
export async function returnBlockImg(url: URL): Promise<Response> {
  const blockImg = await fetch(url.origin + '/static/media/BlockImg.png');
  if (!blockImg.ok) {
    return new Response(null, {
      status: 302,
...
export async function returnWhiteListImg(url: URL): Promise<Response> {
