...
export async function getIPAddress(ip: string): Promise<string> {
  ...
  try {
    ...
    const ipData: any = await ipInfo.json();
    ...
    if (ipInfo.ok && ipData.data) {
      ...
      const addressData: any = await addressInfo.json();
...
// 检查文件扩展名是否有效
export function isExtValid(fileExt: string): boolean {
  return [
 ...
 * @returns 文件扩展名
 */
export function resolveFileExt(fileName: string, fileType = 'application/octet-stream'): string {
 ...
 * @param fileType - 文件 MIME 类型（仅作参考）
...
// 图像审查
export async function moderateContent(env: Env, url: string): Promise<string> {
  ...
  if (uploadModerate.channel === 'moderatecontent.com') {
    ...
    if (apikey == undefined || apikey == null || apikey == '') {
      ...
      try {
        ...
        const moderate_data: any = await fetchResponse.json();
...
// 结束上传：清除缓存，维护索引
export async function endUpload(
...
// 从 request 中解析 ip 地址
export function getUploadIp(request: Request): string | null {
  const ip =
...
// 检查上传IP是否被封禁 - 功能已移除
export async function isBlockedUploadIp(env: Env, uploadIp: string): Promise<boolean> {
  // IP 封禁功能已移除，始终返回 false
  return false;
}

// 构建唯一文件ID
export async function buildUniqueFileId(
  context: PagesContext,
  fileName: string,
  ...
  if (nameType === 'index') {
    baseId = normalizedFolder
      ? `${normalizedFolder}/${unique_index}.${fileExt}`
    ...
    while (true) {
      ...
      const testFullId = normalizedFolder
        ? `${normalizedFolder}/${shortId}.${fileExt}`
        : `${shortId}.${fileExt}`;
  ...
  if ((await db.get(baseId)) === null) {
    // 原子性占位，防止并发使用同一ID
    try {
      await db.put(baseId, '', { expirationTtl: 300 });
    } catch {
      /* 占位失败则由调用方决定 */
    }
    return baseId;
  ...
  while (true) {
    ...
    if ((await db.get(duplicateId)) === null) {
      // 原子性占位，防止并发使用同一ID
      try {
        await db.put(duplicateId, '', { expirationTtl: 300 });
      } catch {
        /* 占位失败则由调用方决定 */
      }
      return duplicateId;
...
// 基于uploadId的一致性渠道选择
export function selectConsistentChannel<T>(channels: T[], uploadId: string, loadBalanceEnabled: boolean): T | undefined {
