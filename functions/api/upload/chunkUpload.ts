...
import type { DiscordAttachmentInfo } from '../../utils/storage/discordAPI.js';
import type { TelegramFileDetails } from '../../utils/storage/telegramAPI.js';
import {
  S3Client,
...
import type { Env, FileMetadata, UploadConfig } from '../../types';

// 初始化分块上传
export async function initializeChunkedUpload(context: UploadContext): Promise<Response> {
  ...
  try {
    ...
    if (uploadChannel === 'telegram') {
      ...
      if (tgChannels.length === 0) {
        ...
        const tgChannel = tgChannels.find((ch: any) => ch.name === channelName);
        if (!tgChannel) {
        ...
        // 检查是否有至少一个启用的渠道
        const enabledChannel = tgChannels.find((ch: any) => ch.enabled !== false);
        if (enabledChannel) {
          channelAvailable = true;
        } else {
      ...
      if (discordChannels.length === 0) {
        ...
        const discordChannel = discordChannels.find((ch: any) => ch.name === channelName);
        if (!discordChannel) {
        ...
        const enabledChannel = discordChannels.find((ch: any) => ch.enabled !== false);
        if (enabledChannel) {
          channelAvailable = true;
        } else {
      ...
      if (s3Channels.length === 0) {
        ...
        const s3Channel = s3Channels.find((ch: any) => ch.name === channelName);
        if (!s3Channel) {
        ...
        const enabledChannel = s3Channels.find((ch: any) => ch.enabled !== false);
        if (enabledChannel) {
          channelAvailable = true;
        } else {
      ...
      if (!env.hammybox_r2) {
        ...
        const r2Channel = r2Channels.find((ch: any) => ch.name === channelName);
        if (!r2Channel) {
        ...
        const enabledChannel = r2Channels.find((ch: any) => ch.enabled !== false);
        if (enabledChannel) {
          channelAvailable = true;
        } else {
    ...
    return createErrorResponse(
      `Failed to initialize chunked upload - ${error.message}`,
      'INIT_FAILED',
      500
    );
  }
...
// 处理客户端分块上传
export async function handleChunkUpload(context: UploadContext): Promise<Response> {
  ...
  try {
    const chunk = formdata.get('file') as unknown as File;
    if (!chunk) {
    ...
    const originalFileType = formdata.get('originalFileType') as string;
    ...
    // 验证会话信息
    if (
      sessionInfo.originalFileName !== originalFileName ||
    ...
    // 获取上传渠道
    const uploadChannel =
      url.searchParams.get('uploadChannel') || sessionInfo.uploadChannel || 'telegram';
    if (uploadChannel === 'webdav') {
      return createErrorResponse(
        'WebDAV channel does not support chunked uploads. Please use non-chunked upload within your Cloudflare request body limit',
        'WEBDAV_CHUNK_NOT_SUPPORTED',
...
// 带超时保护的异步上传分块到存储端
async function uploadChunkToStorageWithTimeout(
  context: UploadContext,
  chunkIndex: number,
  totalChunks: number,
  uploadId: string,
  ...
  chunkData: ArrayBuffer | undefined
): Promise<void> {
  const { env } = context;
  const db = getDatabase(env);
  const chunkKey = `chunk_${uploadId}_${chunkIndex.toString().padStart(3, '0')}`;
  ...
  try {
    ...
    try {
      ...
      if (chunkRecord && chunkRecord.metadata) {
        ...
        const errorMetadata = {
          ...(chunkRecord.metadata as Record<string, unknown>),
        ...
        await db.put(chunkKey, usingD1 ? '' : chunkRecord.value as ArrayBuffer, {
...
async function uploadChunkToStorage(
  ...
  try {
    ...
    if (chunkData !== undefined) {
      ...
      chunkMetadata = chunkRecord && chunkRecord.metadata ? (chunkRecord.metadata as Record<string, unknown>) : {};
    } else {
      // 从数据库读取分块数据和metadata
      const chunkRecord = await db.getWithMetadata(chunkKey, { type: 'arrayBuffer' });
      ...
      chunkData = chunkRecord.value as ArrayBuffer;
      chunkMetadata = chunkRecord.metadata as Record<string, unknown>;
    }
    ...
    for (let retry = 0; retry < MAX_RETRIES; retry++) {
      ...
      if (uploadChannel === 'cfr2') {
        uploadResult = await uploadSingleChunkToR2Multipart(
          context,
          chunkData,
          chunkIndex,
          totalChunks,
        ...
        uploadResult = await uploadSingleChunkToS3Multipart(
          context,
          chunkData,
          chunkIndex,
          totalChunks,
        ...
        uploadResult = await uploadSingleChunkToTelegram(
          context,
          chunkData,
          chunkIndex,
          totalChunks,
        ...
        uploadResult = await uploadSingleChunkToDiscord(
          context,
          chunkData,
          chunkIndex,
          totalChunks,
    ...
    try {
      ...
      if (chunkRecord && chunkRecord.metadata) {
        ...
        await db.put(chunkKey, isD1 ? '' : chunkRecord.value as ArrayBuffer, {
          metadata: errorMetadata,
          expirationTtl: 3600, // 1小时过期
        });
      }
...
async function uploadSingleChunkToR2Multipart(
  context: UploadContext,
  ...
  try {
    ...
    if (chunkIndex === 0) {
      ...
      let multipartInfoData: string | null = null;
      let retryCount = 0;
      ...
      while (!multipartInfoData && retryCount < maxRetries) {
        ...
        if (!multipartInfoData) {
          ...
          retryCount++;
          console.log(
            `R2 chunk ${chunkIndex} waiting for multipart initialization... (${retryCount}/${maxRetries})`
          );
        }
      }
    ...
    // 上传分块
    const multipartUpload = R2DataBase.resumeMultipartUpload(
      finalFileId,
...
async function uploadSingleChunkToS3Multipart(
  context: UploadContext,
  chunkData: ArrayBuffer,
  chunkIndex: number,
  totalChunks: number,
  ...
  try {
    ...
    if (specifiedChannelName) {
      s3Channel = s3Channels.find((ch: any) => ch.name === specifiedChannelName);
    }
    if (!s3Channel) {
    ...
    console.log(
      `Uploading S3 chunk ${chunkIndex} for uploadId: ${uploadId}, selected channel: ${
        s3Channel.name || 'default'
      }`
    );
    ...
    if (chunkIndex === 0) {
      ...
      while (!multipartInfoData && retryCount < maxRetries) {
        ...
        if (!multipartInfoData) {
          ...
          retryCount++;
          console.log(
            `S3 chunk ${chunkIndex} waiting for multipart initialization... (${retryCount}/${maxRetries})`
          );
        }
      }
...
// 上传单个分块到Telegram
async function uploadSingleChunkToTelegram(
  context: UploadContext,
  chunkData: ArrayBuffer,
  chunkIndex: number,
  totalChunks: number,
  ...
  try {
    ...
    let tgChannel: any;
    if (specifiedChannelName) {
      tgChannel = tgChannels.find((ch: any) => ch.name === specifiedChannelName);
    }
    if (!tgChannel) {
    ...
    console.log(
      `Uploading Telegram chunk ${chunkIndex} for uploadId: ${uploadId}, selected channel: ${
        tgChannel.name || 'default'
      }`
    );
...
// 上传单个分块到Discord
async function uploadSingleChunkToDiscord(
  context: UploadContext,
  chunkData: ArrayBuffer,
  chunkIndex: number,
  totalChunks: number,
  ...
  try {
    ...
    if (specifiedChannelName) {
      discordChannel = discordChannels.find((ch: any) => ch.name === specifiedChannelName);
    }
    if (!discordChannel) {
      discordChannel = selectConsistentChannel(
        discordChannels,
        uploadId,
        discordSettings.loadBalance?.enabled
      );
    }
    ...
    console.log(
      `Uploading Discord chunk ${chunkIndex} for uploadId: ${uploadId}, selected channel: ${
        discordChannel.name || 'default'
      }`
    );
...
async function uploadChunkToDiscordWithRetry(
  botToken: string,
  channelId: string,
  chunkBlob: Blob,
...
export interface ChunkStatusItem {
  ...
  chunkSize?: unknown;
  uploadTime?: unknown;
  uploadStartTime?: unknown;
  timeoutThreshold?: unknown;
  uploadChannel?: unknown;
  isTimeout?: boolean;
}
...
export async function retryFailedChunks(
  ...
  console.log(
    `Starting concurrent retry for ${failedChunks.length} failed chunks with max concurrency: ${maxConcurrency}`
  );
  ...
  for (let i = 0; i < chunksToRetry.length; i += batchSize) {
    ...
    console.log(
      `Processing batch ${Math.floor(i / batchSize) + 1}: chunks ${batch.map(c => c.index).join(', ')}`
    );
    ...
    // 限制并发数量
    const batchResults: RetryResult[] = [];
      ...
      for (const result of concurrentResults) {
        if (result.status === 'fulfilled') {
          ...
          batchResults.push({
            success: false,
            chunk: null,
            error: (result.reason as Error)?.message || 'Task failed',
            reason: 'task_error',
          });
        }
      }
  ...
  console.log(
    `Retry completed: ${successCount} successful, ${failureCount} failed out of ${results.length} chunks`
  );
...
// 重试单个失败的分块
async function retrySingleChunk(
  context: UploadContext,
  ...
  try {
    ...
    const chunkData = chunkRecord.value as ArrayBuffer;
    const chunkMetadata = (chunkRecord.metadata || {}) as Record<string, unknown>;
    const originalFileName = chunkMetadata.originalFileName || 'unknown';
    const originalFileType = chunkMetadata.originalFileType || 'application/octet-stream';
    const uploadId = chunkMetadata.uploadId as string;
    const totalChunks = (chunkMetadata.totalChunks as number) || 1;
    ...
    while (retryCount < maxRetries) {
      ...
      const retryPromise = (async () => {
        if (uploadChannel === 'cfr2') {
          return await uploadSingleChunkToR2Multipart(
            context,
            chunkData,
            chunk.index,
            totalChunks,
          ...
          return await uploadSingleChunkToS3Multipart(
            context,
            chunkData,
            chunk.index,
            totalChunks,
          ...
          return await uploadSingleChunkToTelegram(
            context,
            chunkData,
            chunk.index,
            totalChunks,
          ...
          return await uploadSingleChunkToDiscord(
            context,
            chunkData,
            chunk.index,
            totalChunks,
      ...
      if (uploadResult && (uploadResult as Record<string, unknown>).success) {
        // 更新状态为成功
        const updatedMetadata = {
          ...chunkMetadata,
          status: 'completed',
        ...
        throw new Error(
          ((uploadResult as Record<string, unknown>)?.error as string) || 'Unknown retry error'
        );
      }

      retryCount++;
      lastError = new Error(
        ((uploadResult as Record<string, unknown>)?.error as string) || 'Unknown error'
      );
      console.warn(`Chunk ${chunk.index} retry ${retryCount} failed: ${lastError.message}`);
    }
  } catch (error: any) {
    lastError = error;
    const isTimeout = error.message === 'Retry timeout';
    console.warn(
      `Chunk ${chunk.index} retry ${retryCount} ${isTimeout ? 'timed out' : 'failed'}: ${error.message}`
    );
    ...
    try {
      ...
      if (chunkRecord) {
        ...
        // D1模式下不保存二进制数据，避免SQLITE_TOOBIG
        await db.put(chunk.key, isD1Retry ? '' : chunkRecord.value as ArrayBuffer, {
...
export async function cleanupFailedMultipartUploads(
  context: UploadContext,
  uploadId: string,
  uploadChannel: string
): Promise<void> {
...
// 检查分块上传状态
export async function checkChunkUploadStatuses(
  env: Env,
  uploadId: string,
  totalChunks: number
): Promise<ChunkStatusItem[]> {
  const chunkStatuses: ChunkStatusItem[] = [];
  ...
  for (let i = 0; i < totalChunks; i++) {
    ...
    try {
      ...
      if (chunkRecord && chunkRecord.metadata) {
        ...
        if (
          status === 'uploading' &&
          metadata.timeoutThreshold &&
          currentTime > (metadata.timeoutThreshold as number)
        ) {
          status = 'timeout';
          ...
          // D1模式下不保存二进制数据，避免SQLITE_TOOBIG
          await db
            .put(chunkKey, isD1Status ? '' : chunkRecord.value as ArrayBuffer, {
        ...
        let hasData = false;
        const value = chunkRecord.value as ArrayBuffer | null;
        if (status === 'completed') {
          // 已完成的分块，不存储原始数据
          hasData = false;
        } else if (status === 'uploading' || status === 'failed' || status === 'timeout') {
          // 正在上传、失败或超时的分块通过原始数据判断
          hasData = !!(value && value.byteLength > 0);
        } else {
          // 其他状态也检查是否有数据
          hasData = !!(value && value.byteLength > 0);
        }
        ...
        chunkStatuses.push({
          ...
          uploadStartTime: metadata.uploadStartTime,
          timeoutThreshold: metadata.timeoutThreshold,
          uploadChannel: metadata.uploadChannel,
...
// 清理临时分块数据
export async function cleanupChunkData(env: Env, uploadId: string, totalChunks: number): Promise<void> {
  try {
    const db = getDatabase(env);
...
// 清理上传会话
export async function cleanupUploadSession(env: Env, uploadId: string): Promise<void> {
  try {
    const db = getDatabase(env);
...
export async function forceCleanupUpload(
  context: UploadContext,
  uploadId: string,
  totalChunks: number
): Promise<void> {
  ...
  try {
    ...
    for (let i = 0; i < totalChunks; i++) {
      ...
      cleanupPromises.push(
        db.delete(chunkKey).catch(err => console.warn(`Failed to delete chunk ${i}:`, err))
      );
    }

    // 清理相关的键
    const keysToCleanup = [`upload_session_${uploadId}`, `multipart_${uploadId}`];

    keysToCleanup.forEach(key => {
      cleanupPromises.push(
        db.delete(key).catch(err => console.warn(`Failed to delete key ${key}:`, err))
      );
    });
...
export async function uploadLargeFileToTelegram(
  context: UploadContext,
  file: File,
  fullId: string,
  ...
  const chunks: Array<{ index: number; fileId: string; size: number; fileName: string }> = [];
  ...
  try {
    ...
    return createErrorResponse(
      `Telegram Channel Error: Large file upload failed - ${error.message}`,
...
async function uploadChunkToTelegramWithRetry(
  ...
  totalChunks: number,
  maxRetries = 2
): Promise<TelegramFileDetails | null> {
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
