...
import type { Env, PagesContext, FileMetadata, SecurityConfig, UploadConfig } from '../../types';

export async function onRequest(context: UploadContext): Promise<Response> {
  // Contents of context object
  const { request, env, waitUntil, params, next, data } = context;
  ...
  const uploadConfig = await fetchUploadConfig(env as Env, context);
  ...
  // 判断上传ip是否被封禁
  const isBlockedIp = await isBlockedUploadIp(env as Env, uploadIp || '');
  if (isBlockedIp) {
  ...
  if (cleanupRequest) {
    ...
    return await handleCleanupRequest(context as UploadContext, uploadId, totalChunks);
  }

  // 检查是否为初始化分块上传请求
  const initChunked = url.searchParams.get('initChunked') === 'true';
  if (initChunked) {
    return await initializeChunkedUpload(context as UploadContext);
  }
  ...
  if (isChunked) {
    if (isMerge) {
      return await handleChunkMerge(context as UploadContext);
    } else {
      return await handleChunkUpload(context as UploadContext);
    }
  }

  // 处理非分块文件上传
  return await processFileUpload(context as UploadContext);
}

// 通用文件上传处理函数
async function processFileUpload(context: UploadContext, formdata: FormData | null = null): Promise<Response> {
  ...
  const fileSizeBytes = fileBlob.size; // 文件大小，单位字节
  ...
  if (fileType.startsWith('image/')) {
    try {
      ...
      const headerBuffer = await fileBlob.slice(0, 65536).arrayBuffer();
  ...
  if (uploadChannel === 'CloudflareR2') {
    ...
    // ----------------Telegram New 渠道-------------------
    return await uploadFileToTelegram(
      context,
      fullId,
      metadata,
      fileExt,
...
async function uploadFileToCloudflareR2(
  ...
  // 检查R2数据库是否配置
  if (typeof env.hammybox_r2 == 'undefined' || env.hammybox_r2 == null || (env.hammybox_r2 as unknown) == '') {
  ...
  if (specifiedChannelName) {
    r2Channel = r2Settings.channels.find((ch: any) => ch.name === specifiedChannelName);
  }
  if (!r2Channel) {
...
async function uploadFileToTelegram(
  ...
  if (fileExt === 'gif') {
    ...
    const newFile = new File([formdata.get('file') as unknown as Blob], newFileName, { type: fileType });
  ...
  // 选择对应的发送接口
  const fileTypeMap: Record<string, { url: string; type: string }> = {
  ...
  let sendFunction: { url: string; type: string } = Object.keys(fileTypeMap).find(key =>
    fileType.startsWith(key)
  )
    ? fileTypeMap[
        Object.keys(fileTypeMap).find(key => fileType.startsWith(key)) as string
      ]
    : defaultType;
  ...
  if (sendFunction.url === 'sendPhoto' || sendFunction.url === 'sendAnimation') {
    ...
    // 检查图片尺寸（Telegram 限制：宽+高 ≤ 10000）
      ...
      metadata.Width &&
      metadata.Height &&
      (metadata.Width as number) + (metadata.Height as number) > 10000
    ) {
      console.log(
        `[Telegram] Image dimensions too large (${metadata.Width}x${metadata.Height}), switching to sendDocument`
      );
      sendFunction = { url: 'sendDocument', type: 'document' };
    }
  }
  ...
  // 上传文件到 Telegram
  let res: Response = createErrorResponse(
    'Telegram upload failed, check your environment params about telegram channel',
    'TELEGRAM_UPLOAD_FAILED',
    400
  );
  try {
    const response = await telegramAPI.sendFile(
      formdata.get('file') as unknown as Blob,
      tgChatId,
      sendFunction.url,
      sendFunction.type
    );
...
async function uploadFileToExternal(
  ...
  metadata.ExternalLink = String(extUrl);
  // 写入KV数据库
  try {
    await db.put(fullId, '', {
      metadata: metadata,
...
async function uploadFileToDiscord(
  ...
  if (!discordChannel || !discordChannel.botToken || !discordChannel.channelId) {
    return createErrorResponse(
      'Discord channel not properly configured',
  ...
  try {
    // 上传文件到 Discord
    const response = await discordAPI.sendFile(fileBlob, discordChannel.channelId, fileName);
...
// 上传到 HuggingFace
async function uploadFileToHuggingFace(
  context: UploadContext,
  fullId: string,
  metadata: FileMetadata,
  returnLink: string
  ...
  if (!hfSettings || !hfSettings.channels || hfSettings.channels.length === 0) {
    ...
    return createErrorResponse(
      'No HuggingFace channel configured',
  ...
  let hfChannel: any;
  if (specifiedChannelName) {
    hfChannel = hfChannels.find((ch: any) => ch.name === specifiedChannelName);
  }
  if (!hfChannel) {
  ...
  if (!hfChannel || !hfChannel.token || !hfChannel.repo) {
    ...
    return createErrorResponse(
      'HuggingFace channel not properly configured',
  ...
  try {
    ...
    const result = await huggingfaceAPI.uploadFile(
      fileBlob,
      hfFilePath,
      `Upload ${fileName}`,
      precomputedSha256 as string | null
    );
    ...
    return createErrorResponse(
      `HuggingFace upload failed - ${error.message}`,
      'HUGGINGFACE_UPLOAD_FAILED',
      500
    );
  }
}
...
async function uploadFileToWebDAV(
  ...
  let webdavChannel: any;
  if (specifiedChannelName) {
    webdavChannel = webdavChannels.find((ch: any) => ch.name === specifiedChannelName);
  }
  if (!webdavChannel) {
  ...
  if (!webdavChannel || !webdavChannel.baseUrl) {
    return createErrorResponse(
      'WebDAV channel not properly configured',
  ...
  try {
    ...
    await webdavAPI.putFile(fullId, fileBlob, fileBlob.type || metadata.FileType || 'application/octet-stream');
