...
export async function onRequest(context: PagesContext): Promise<Response> {
  ...
  // 读取其他设置
  othersConfig = await fetchOthersConfig(env as Env);
  ...
  const allowedDirListFormatted: string[] = [];
  ...
  // 从params中读取返回的文件类型
  let fileType: string | string[] = requestUrl.searchParams.get('content');
  if (fileType == null) {
    fileType = ['image'];
  } else {
    fileType = (fileType as string).split(',');
  }
  ...
  for (let i = 0; i < allowedDirListFormatted.length; i++) {
    if (
      allowedDirListFormatted[i] === '' ||
      dir === allowedDirListFormatted[i] ||
      dir.startsWith(allowedDirListFormatted[i] + '/')
    ) {
      dirAllowed = true;
      break;
    }
  ...
  let allRecords: any[] = await getRandomFileList(context, requestUrl, dir);

  // 筛选出符合fileType要求的记录
  allRecords = allRecords.filter(item => {
    return (fileType as string[]).some(type => item.FileType?.includes(type));
  });
  ...
  if (includeTags.length > 0) {
    allRecords = allRecords.filter(item => {
      const fileTags = (item.Tags || []).map((t: unknown) => String(t).toLowerCase());
...
async function getRandomFileList(context: PagesContext, url: URL, dir: string): Promise<any[]> {
  ...
  await (cache.put as any)(
    cacheKey,
    new Response(JSON.stringify(allRecords), {
      headers: {
