/**
 * 自动扫描 functions/ 目录，生成 deploy/worker/index.ts
 *
 * 扫描规则（与 Cloudflare Pages Functions 一致）：
 * - _middleware.js/_middleware.ts → 中间件，按目录层级链式执行
 * - index.js/index.ts             → 映射到所在目录路径
 * - [[path]].js/[[path]].ts       → catch-all 路由
 * - xxx.js/xxx.ts                 → 精确路由
 * - utils/ 目录跳过（工具模块，不是路由）
 * - 不含 onRequest 导出的文件跳过（工具模块）
 *
 * 使用方式: npx tsx deploy/worker/generate-routes.ts
 */
 ...
 */
function hasOnRequestExport(filePath: string): boolean {
  const content = readFileSync(filePath, 'utf8');
  return (
    /export\s+(async\s+)?function\s+onRequest/m.test(content) ||
 ...
 */
function toUrlPath(filePath: string): { urlPath: string; isCatchAll: boolean } {
 ...
 *
 * 生成的 import 统一使用 .js 后缀（即使源文件是 .ts）：
 * Cloudflare Wrangler/esbuild 打包时会将 .js 后缀解析到对应的 .ts 源文件，
 * 与 functions/ 内部 import 的书写风格保持一致。
 */
 ...
 */
function getMiddlewareChain(dirPath: string): string[] {
  const chain: string[] = [];
...
function scanDir(dir: string): void {
  ...
  for (const entry of entries) {
    ...
    // 中间件（.ts 与 .js 同名文件共存时，只取 .ts）
    const baseName = entry.replace(/\.(js|ts)$/, '');
    if (baseName === '_middleware') {
      if (entry.endsWith('.js') && existsTs(fullPath)) continue; // 已有 .ts 版本，跳过 .js
 ...
 * 判断同目录下是否存在同名 .ts 文件
 */
function existsTs(jsPath: string): boolean {
...
for (const route of routes) {
  const mwArray =
    route.middlewareVarNames && route.middlewareVarNames.length > 0
...
console.log(`Generated deploy/worker/index.ts`);
